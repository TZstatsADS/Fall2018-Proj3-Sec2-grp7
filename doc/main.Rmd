---
title: "Project 3 - Baseline"
author: "Yixin Zhang, Oded Loewenstein"
...
We chose depth = 2 based on the MSE. (To avoid rerun cross-validation part, we set depth=2 manually.)
```{r best_model}
...
par_best <- list(depth=2)
```

* Train the model with the entire training set using the selected model (model parameter) via cross-validation.
```{r final_train, warning=FALSE, message=FALSE}
tm_train=NA
...
We write a img function to get a list of psnr of all the images.
Then calculate the mean to get PSNR = 24.67765. 
```{r}
source("../lib/psnr.R")
